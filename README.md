...
**Реестр моделей.** На старте предупреждает, если модель сессии не из известного списка (по умолчанию — текущие семейства Claude: `opus`, `sonnet`, `haiku`, `fable`); вышла новая модель — разовый нудж, после чего вы обновляете список. Реактивная проверка включена по умолчанию. Отдельное периодическое напоминание «пересверить линейку» включается датой в `model_registry_verified_on` (по умолчанию выкл).
