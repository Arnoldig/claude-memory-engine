...
DEFAULT_MESSAGES = {
    ...
    "frontmatter.unparsed_hint_date": "Dates must be strict ISO `YYYY-MM-DD` (e.g. 2026-01-01) — no other format is understood. Until fixed, this field does nothing: a `reverify_after` never comes due, an `archived_on` never reaches retention.",
    ...
    "staleness.pending_file.unparsed_hint": "Fix the value in the lesson frontmatter. `applies_to`: inline list `[\"a/b.py\"]`, YAML list of `- ` items, or a single glob string. Dates (`reverify_after`, `archived_on`): strict ISO `YYYY-MM-DD`. An empty value means the field was declared but never filled in.",
