...
DEFAULT_MESSAGES = {
    ...
    "self_check.close_pattern_lag": "[config self-check] `{field}` does not recognise every GitHub closing keyword — missing: {missing}. GitHub closes an issue on any of the nine keywords, so a commit like `{example}` will close the task while the lesson gate stays silent, and 'did not recognise' is indistinguishable from 'no closure happened'. This usually means the pattern was copied from an older default and froze: the current library default covers all nine, so rebuild your additions on top of it.",
