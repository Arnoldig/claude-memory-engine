...
DEFAULT_MESSAGES = {
    ...
    "bash.commit_review": "[memory] Before `git {action}`: run a security review and a simplification pass over the changed files ({files}), fix what they report right away, and only then save or send. When a finding has more than one reasonable fix, ask instead of guessing. Already reviewed in this session? Ignore this line.",
    "bash.commit_review_more": " (+{rest} more)",
    "bash.issue_formulation_header": "[memory] The issue title or body looks mechanically off:",
    "bash.issue_title_leading_hash": "the title starts with `#` — GitHub reads that as a reference to another issue; drop the leading `#`.",
    "bash.issue_title_looks_like_slug": "the title is a slug — write a human sentence that starts with a verb.",
    "bash.issue_title_has_path": "the title carries a file name or a path — a title should read without technical detail.",
    "bash.issue_title_has_acronym": "the title has an all-caps acronym — spell it out in plain words.",
    "bash.issue_body_long_without_context": "the body is over {limit} characters and points at no context file — move the long part into `{context_dir}` and link it from the body.",
    ...
    "bloat.core_wall_over": "[memory] {core_file}: {size} bytes — PAST the platform wall of {wall} bytes. Claude Code loads only the first {wall} bytes (or first 200 lines) of this file, so its tail is ALREADY missing from context, silently. Trim now: move older entries into lessons.",
    "bloat.core_wall_warn": "[memory] {core_file}: {size} bytes ({pct}% of the {wall}-byte platform wall) — approaching the hard cutoff. Past it, Claude Code silently drops the tail of this file from context (it loads the first {wall} bytes or 200 lines, whichever ends first).",
    ...
    "bloat.empty_name": "[memory] {filename}: `name` is empty. Put a short latin slug there (the file name is usually right). Do not put a human title in `name`: the host memory rewrites that field into a latin slug when the lesson file is created, so a non-latin title cannot survive there. Words you want this lesson to be FOUND by belong in `keywords:` — the host never touches that field and it carries the same x2 retrieval weight as `name`.",
    "bloat.no_keywords": "[memory] {filename}: `keywords` is empty and `name` carries no word of the language this catalog is written in, so the high-weight retrieval field (name + keywords, x2) is empty and a query in that language cannot match it. Add `keywords:` in the language you write lessons in.",
    ...
    "concurrency.conflict_reason": "Memory file {filename} was changed by another session after you last read or wrote it (a parallel session wrote in between). To avoid overwriting that change: re-read the file (Read), then repeat your edit — it will go through. [memory-concurrency-guard]",
    ...
    "diag.no_name": "  with an empty `name` (put a latin slug there): {count}",
    "diag.no_keywords": "  with an empty high-weight retrieval field (no `keywords`, latin-only `name`): {count}",
    "diag.no_keywords_item": "      - {f}",
    "diag.no_name_item": "      - {f}",
    ...
    "health.no_name": "{nn} lessons with an empty `name` (put a short latin slug there; words to be found by belong in `keywords`)",
    "health.no_keywords": "{nk} lessons whose high-weight retrieval field is empty for this catalog's language (fill `keywords:` — `name` is a latin slug and cannot carry it)",
    ...
    "subagent.start_catalog": "  • lesson catalog: {catalog}",
    "sync_guards.installed": "{name}: installed.",
    "sync_guards.updated": "{name}: updated to the packaged version.",
    "sync_guards.unchanged": "{name}: already up to date.",
    "sync_guards.registered": "settings.json: +{added} guard hook registration(s) (duplicates skipped).",
    "subagent.start_header": "[memory:subagent] You are a subagent: project rules and memory lessons are NOT loaded into your context, and the engine's retrieval never reaches you. Open what you need BEFORE acting.",
    "subagent.start_rules": "  • project rules: {rules}",
    ...
    "self_check.guard_drift": "[config self-check] {path} differs from the guard shipped with this engine version — hand-copied guards drift silently. Run `claude-memory sync-guards` to update it, or keep your local edition knowingly (this note appears only in doctor/verbose).",
    "self_check.hook_cmd_missing": "[config self-check] hooks in {scope}: the {event} command points at `{path}`, and that file does not exist — the hook NEVER runs, and a hook that fails to launch looks exactly like a hook that had nothing to say. Fix the path (project moved? account renamed?) or re-run `claude-memory init`.",
    "self_check.hook_interp_broken": "[config self-check] {wrapper} pins interpreter `{interp}`, and `import claude_memory` fails under it — EVERY engine hook dies at startup, silently. Reinstall the package for that interpreter (`{interp} -m pip install claude-memory-engine`) or re-run `claude-memory init`.",
    ...
    "stale_reconcile.guard.core_wall": "core-wall",
    "stale_reconcile.guard.archive_age": "archive-age",
    "stale_reconcile.guard.lesson_count": "lesson-count",
    "stale_reconcile.guard.no_keywords": "search-keywords",
    "stale_reconcile.guard.commit_review": "pre-commit-review",
    "stale_reconcile.guard.issue_formulation": "issue-wording",
