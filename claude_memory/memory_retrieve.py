...
# Алфавит токенизатора РАЗДЕЛЁН на две части намеренно, и деление несущее.
# `_TOKEN_LATIN` — латиница, цифры, подчёркивание. `_TOKEN_NON_LATIN` — буквы других
# алфавитов, которые ретривер УМЕЕТ превращать в стемы (сегодня только кириллица).
# По второй части считается признак «высокий ярус (name + keywords) пуст для языка
# каталога» — ими пользуются подсказка при записи урока (hooks_cli) и сводка здоровья
# (catalog_generate). Один источник истины нужен вот зачем: добавляя сюда новый алфавит,
# вы одновременно учите ретривер его индексировать И подсказку про него говорить. Две отдельные
# константы разошлись бы молча, и движок советовал бы заполнить `keywords` на языке,
# слова которого он всё равно не проиндексирует, — то есть давал бы заведомо ложный совет.
# Набор символов ТОТ ЖЕ, что был литералом до 0.22.0 (порядок внутри класса значения не
# имеет), поэтому стемы не меняются и `_PARSER_LOGIC_VERSION` поднимать не нужно.
_TOKEN_LATIN = "a-zA-Z0-9_"
_TOKEN_NON_LATIN = "а-яёА-ЯЁ"
_TOKEN_RE = re.compile(f"[{_TOKEN_LATIN}{_TOKEN_NON_LATIN}]+")
_NON_LATIN_RE = re.compile(f"[{_TOKEN_NON_LATIN}]")


def has_indexable_non_latin(text: str) -> bool:
    """Есть ли в тексте буквы не-латинского алфавита, которые ретривер индексирует.

    Признак «каталог ведётся не на английском» в терминах ТОГО ЖЕ алфавита, которым
    считаются стемы: спрашивать про буквы, которых токенизатор не знает, бессмысленно —
    заполнение `keywords` на таком языке до индекса всё равно не дойдёт.
    """
