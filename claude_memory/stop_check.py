...
# Идентификатор зонда — В ОДНОМ месте. Раньше он стоял литералом дважды: в шаблонах
# выше и в сверке `!= "42"` у потребителя. Две копии одной величины расходятся молча,
# и тогда страж начинает мерить не то: жалуется на верный шаблон и одновременно
# маскирует настоящую дыру (воспроизведено прогоном при попытке добавить третью ось).
# Ожидаемый ответ берут ОТСЮДА, а не пишут числом рядом со сверкой.
GITHUB_CLOSE_PROBE_ID = "42"

# Третья координата эталона: РЕГИСТР слова. Документирован наравне с двоеточием —
# «The keywords can be followed by colons or in uppercase» (`CLOSES #10`, `CLOSES: #10`),
# а строчное написание стоит в примере GitHub на несколько задач: `resolves #123`.
#
# Зачем ось нужна именно стражу отставания. У него есть выход «промахнулись ВСЕ зонды =
# законная полная замена». Пока зонды идут в одном регистре, копия дефолта, потерявшая
# `(?i)`, промахивается на всех — и выглядит законной заменой, хотя узнаёт лишь треть
# документированных форм. Ось разводит эти случаи: часть зондов проходит, часть нет —
# значит шаблон отстал, а не заменён. Воспроизведено 2026-07-20 (заявка #20).
#
# Пара — (имя для человека, преобразование слова). Перечень форм растёт произведением
# осей, поэтому число зондов нигде не записывают числом: оно считается по осям.
GITHUB_CLOSE_CASES: tuple = (
    ("как в документации", str),
    ("ВЕРХНИЙ", str.upper),
    ("строчный", str.lower),
)
