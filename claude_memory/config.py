...
class MemoryConfig:
    ...
    #
    # ЧЕМ ЭТО ОТЛИЧАЕТСЯ ОТ БЮДЖЕТА ЯДРА, и почему разница принципиальна.
    # У авто-памяти (`core_file`) порог ЖЁСТКИЙ и чужой: Claude Code грузит первые 200
    # строк ИЛИ первые 25 КБ, что раньше, остальное в контекст не попадает вовсе. У файла
    # инструкций такого порога НЕТ — документация Claude Code говорит дословно, что он
    # «loaded in full regardless of length». Значит здесь мы стережём не механику, а
    # КАЧЕСТВО: чем больше правил в контексте, тем больше их модель молча пропускает.
    #
    # Отсюда три следствия, каждое воплощено в коде:
    #   1) ОДНА точка срабатывания, без пары warn/over и без `*_warn_ratio`. Пара порогов
    #      вокруг ОЦЕНКИ — псевдоточность: она внушает, будто у числа есть механический
    #      смысл, и подталкивает следующую сессию резать файл ДО числа. Резать надо по цене
    #      молчаливого пропуска правила, а не до цифры.
    #   2) Знаки, а НЕ строки и не байты, и ручки «chars|bytes» тут нет. Строки не меряют
    #      ничего: в стиле «абзац — одна строка» одна строка бывает на тысячи знаков (замер
    #      на живом проекте: 172 строки при 59 000 знаков). Байты врут вдвое на кириллице.
    #   3) Только ПРЕДУПРЕЖДЕНИЕ, никогда не блокировка. Крупный проект законно выходит за
    #      ориентир: цена превышения — размен, и делает его человек, а не движок.
    #
    # 20 000 — это рекомендация Anthropic «до 200 строк», пересчитанная в знаки при обычной
    # длине строки. ОЦЕНКА, а не порог; в документации применительно к файлу инструкций
    # такого числа нет. 0 — страж выключен.
    instructions_budget_chars: int = 20000
    # Какие файлы считать инструкциями проекта. Пути ОТНОСИТЕЛЬНЫЕ; разрешаются от корня
    # проекта, от рабочего каталога сессии и от всех его каталогов-предков — так же, как
    # сам Claude Code собирает файлы инструкций, идя по дереву вверх (см.
    # `hooks_cli.instructions_roots`; без обхода вверх сессия, открытая в подкаталоге
    # worktree, теряла оба канала стража молча).
    # Список, а не одно имя, намеренно: страж, узнающий объект по единственной форме записи,
    # молча слепнет на всех прочих (`CLAUDE.local.md`, `.claude/CLAUDE.md`, `docs/CLAUDE.md`).
    # Дефолт покрыл все 7 файлов из 7 на живом замере; проект с иной раскладкой дописывает
    # свои строкой в конфиг, без правки кода.
    # ГРАНИЦА, НАЗВАННАЯ ВСЛУХ: обхода ВНИЗ нет. Файлы инструкций во вложенных
    # подкаталогах Claude Code при старте не грузит — он подхватывает их, только когда
    # сессия трогает файл оттуда, поэтому их размер стоит иначе.
    instructions_files: Tuple[str, ...] = ("CLAUDE.md",)
    ...
    # Шаблон коммита-закрытия. ИСТОЧНИК ИСТИНЫ по перечню форм — `stop_check.
    # GITHUB_CLOSE_KEYWORDS`; текст ниже лишь поясняет, тесты сверяются с константой.
    # Если ваш проект РАСШИРЯЕТ этот шаблон (добавляет свои формы), помните: копия
    # замерзает в день копирования, а дефолт может вырасти. С 0.14.0 отставание ловит
    # `self_check.close_pattern_lag_issues` — он скажет вслух, каких форм шаблон не знает.
    # По умолчанию — ВСЕ девять официальных слов-закрытий GitHub:
    ...
    #
    # ДВЕ ВЕТКИ ПО НАПИСАНИЮ (0.15.0, заявка #13). GitHub принимает и `Closes #42`, и
    # `Closes: #42` — двоеточие документировано дословно. До 0.15.0 вторая форма молча не
    # узнавалась: GitHub задачу закрывал, привратник урока молчал. Тот же класс, что семья
    # `resolve` в 0.10.0, только по другой координате — не СЛОВО, а НАПИСАНИЕ.
    #
    # ПОЧЕМУ ПОСЛЕ ДВОЕТОЧИЯ ТОЛЬКО ЧИСЛО, а после пробела — число ИЛИ слаг. Это не
    # придирка, а измеренная защита. Слаг (`#memory-lib-cutover`) — конвенция САМОГО
    # движка: GitHub на нечисловой id не реагирует вовсе. А `слово: #слаг` в начале темы
    # коммита — это Conventional Commits (`fix: #тема — что сделано`), и таких тем у
    # живого проекта-потребителя 619. Замер на его истории (2258 коммитов): ветка,
    # принимающая после двоеточия любой id, даёт 8 ложных закрытий, ветка «только число» —
    # 0. Цена ложного тут не косметическая: Stop блокируется, пока не найдётся урок про
    # несуществующую задачу. Числовая семантика после двоеточия ровно зеркалит GitHub —
    # ветка существует потому, что документирована им, и наследует его правила.
    #
    # ГРАНИЦА, НАЗВАННАЯ ВСЛУХ: `\s*` после двоеточия принимает и слитную форму
    # `Closes:#42`, которой в документации GitHub НЕТ (там все примеры с пробелом).
    # Принято намеренно и осознанно ШИРЕ документации: намерение автора в такой записи
    # недвусмысленно, а цена ошибки асимметрична — лишний вопрос про урок виден, объясним
    # и гаснет сам, тогда как пропущенное закрытие невидимо, и это класс всей заявки.
    # В ЭТАЛОН (`stop_check.GITHUB_CLOSE_SYNTAXES`) слитная форма НЕ внесена: требовать от
    # чужого шаблона больше, чем документирует GitHub, библиотека не вправе.
