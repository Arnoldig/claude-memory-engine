...
class MemoryConfig:
    ...
    # Кто полагался на прежний дефолт — задаёт явно (одна строка в конфиге); это названо
    # ЛОМАЮЩИМ в CHANGELOG, и `self_check` предупреждает при живых русских карточках.
