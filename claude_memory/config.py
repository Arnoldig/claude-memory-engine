...
class MemoryConfig:
    ...
    # Подсказка «высокий ярус поиска (name + keywords, ×2) пуст для языка каталога». Ручка
    # булева, а не пороговая: величины, которую можно ослабить, тут нет — либо ярус несёт
    # слова языка каталога, либо нет. Включён по умолчанию: страж, который надо сперва
    # найти и включить, до того, кто за памятью не следит, не доходит. Выключение осмысленно
    # для каталога, который сознательно живёт без `keywords`: там подсказка приходила бы на КАЖДУЮ
    # правку любого старого урока, а шумного стража выключают вместе с защитой.
    no_keywords_nudge_enabled: bool = True

    # — мягкие подсказки перед командой оболочки (bash_notices) —
    # Обе НИЧЕГО НЕ БЛОКИРУЮТ: и пропущенное ревью, и неудачный заголовок задачи
    # обратимы, а блокирующий разбор произвольной команды оболочки ошибался бы часто
    # и был бы снят вместе с защитой.
    commit_review_enabled: bool = True
    # Глаголы git, перед которыми напоминаем про ревью. Пустой набор = выключено.
    commit_review_verbs: Tuple[str, ...] = ("commit", "push")
    # Расширения, считающиеся кодом. Меняется правописание — меняется и охват.
    commit_review_extensions: Tuple[str, ...] = (
        "py", "js", "ts", "tsx", "jsx", "go", "rs", "rb", "java", "kt", "php",
        "sh", "html", "css", "scss", "sql", "yaml", "yml", "json", "toml",
    )
    # Сколько имён файлов показать; 0 = все. Длинный перечень перестают читать.
    commit_review_max_files: int = 10
    issue_formulation_enabled: bool = True
    # Длиннее — подсказываем вынести контекст задачи в отдельный файл. 0 = выкл.
    issue_body_warn_chars: int = 280
    # Каталог файлов контекста задач (у каждого проекта свой). Пусто = проверка выкл:
    # советовать положить файл в каталог, которого в проекте нет, — заведомо ложный совет.
    issue_context_dir: str = ""
    # Порог числа уроков для подсказки «проверь дубли» в пульсе здоровья (0 — выкл). Не про
    ...
    # вышла новая → разовая подсказка → обнови список. Пусто → проверка ВЫКЛ. Периодическое
    ...
    # (model_registry_guard.stale_nudge из hooks_cli; max_age_days=0 = выключено).
    ...
    # Раннее предупреждение о ядре: сказать уже при core_warn_ratio·бюджета (а не только сверх
    # 100%). None — выключить раннее предупреждение (только превышение бюджета).
    core_warn_ratio: Optional[float] = 0.8
    # СТЕНА ПЛАТФОРМЫ для ядра — отдельно от бюджета качества выше. Бюджет — оценка
    # «сколько ядра полезно» (в знаках, решение осознанное: строка бывает сколь угодно
    # длинной, и знаки её ловят). Стена — механика хозяина: Claude Code грузит ПЕРВЫЕ
    # 200 строк ИЛИ ПЕРВЫЕ 25 КБ (25*1024 = 25600 байт), что наступит раньше; остальное
    # молча не попадает в контекст (сверено 2026-08-17, code.claude.com/docs/en/memory.md).
    # Кириллица в UTF-8 весит 2 байта на букву, поэтому знаковый бюджет стену НЕ заменяет:
    # русское ядро в 15 000 знаков весит до 30 000 байт — за стеной ещё до «бюджет
    # превышен». Меряем байты файла (st_size) против значения стены; раннее предупреждение
    # приходит с той же ручкой core_warn_ratio. 0 — проверка выключена.
    core_wall_bytes: int = 25600
...
_TUPLE_FIELDS = {
    ...
    "instructions_files",
    # Списочные поля подсказок перед командой оболочки. Реестр родов ведётся руками
    # только для списков (у остальных род берётся из дефолта), поэтому забыть поле
    # здесь — значит пропустить чужой род молча; ровно это и поймал существующий
    # тест враждебных типов, порождающий случаи из интроспекции полей.
    "commit_review_verbs", "commit_review_extensions",
}
