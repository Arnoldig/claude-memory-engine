...
def test_старые_снимки_подчищаются(tmp_path) -> None:
    """Снимки копятся на каждую правку: за сессию их набралось 28.

    Само по себе это не мешает, но за месяцы накопится столько, что перечень
    станет нечитаемым, а найти в нём нужный — невозможно. Держим предел.
    """
    репо = _репо(tmp_path / "репо")
    for n in range(12):
        (репо / "f.txt").write_text(f"правка {n}\n", encoding="utf-8")
        _вызвать("rm -rf ничего", репо)

    снимки = _снимки(репо)
    assert снимки, "снимки должны создаваться"
    assert len(снимки) <= 10, (
        f"снимков {len(снимки)}: перечень копится без предела и станет нечитаемым")
