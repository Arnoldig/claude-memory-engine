...
def test_old_precedent_archived_recent_kept(cfg) -> None:
    ...
    fb.write_text(
        "**Precedent 2026-01-01:** старый случай, должен уехать.\n\n"
        "**Precedent 2026-06-15:** свежий, остаётся.\n",
        encoding="utf-8",
    )
    ...
    assert "moved to [archive/precedents-2026-Q1.md]" in text   # pointer вместо старого
...
def test_no_candidates_is_noop(cfg) -> None:
    fb = Path(cfg.memory_dir) / "feedback_x.md"
    fb.write_text("**Precedent 2026-06-15:** свежий.\n", encoding="utf-8")
...
def test_count_real_precedents(cfg) -> None:
    text = ("**Precedent 2026-06-15:** живой.\n\n"
            "**Precedent 2026-01-01:** moved to [archive/precedents-2026-Q1.md](x).\n")
