...
def test_non_iterable_topic_order_still_dies_at_load(tmp_path) -> None:
    """ГРАНИЦА, названная вслух: `"topic_order": 42` роняет саму загрузку конфига.

    Это поведение СТАРШЕ заявки #14 — `_coerce` разворачивал значение в кортеж пар и до
    неё, и падал ровно так же; правка про `null` его не касалась. Чинить не стали, чтобы
    не расширять заявку молча, но и делать вид, что случая нет, нельзя: цена у него
    высокая — хуки fail-open проглотят исключение, и движок окажется молча выключен.

    Тест не одобряет поведение, а ЗАКРЕПЛЯЕТ его как известное: когда его починят, он
    упадёт и заставит осознанно переписать эту границу, а не тихо разойтись с реальностью.
    """
    with pytest.raises(TypeError):
        _load(tmp_path, topic_order=42)
