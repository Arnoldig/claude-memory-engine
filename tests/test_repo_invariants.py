...
def test_ci_actually_runs_the_linter() -> None:
    """CI обязан ГОНЯТЬ линтер, а не просто числить его в зависимостях разработки.

    Иначе связка разъезжается молча и в обе стороны: шаг проверки убирают «временно, он
    мешает» — и `ruff` остаётся в `dev`-зависимостях, создавая полную видимость проверки,
    которой нет. Ровно тот класс, вокруг которого построен весь движок: молчание
    неотличимо от «всё чисто».

    Проверяется ПОВЕДЕНИЕ (в каком-то workflow есть запуск `ruff check`), а не строка в
    конкретном файле: расположение шага — дело сопровождающего, факт запуска — нет.
    """
    workflows = list((ROOT / ".github" / "workflows").glob("*.yml"))
    assert workflows, "нет workflow-файлов"
    runs_lint = any(re.search(r"ruff\s+check", _read(w)) for w in workflows)
    assert runs_lint, (
        "ни один workflow не запускает `ruff check` — линтер числится в зависимостях, "
        "но не работает, и это выглядит как «проверено»"
    )


def test_linter_rule_set_stays_narrow_and_targets_the_oldest_python() -> None:
    """Два свойства настройки линтера, каждое из которых ломается молча.

    (1) Набор правил узкий — только молчаливые поломки (`E9`, `F`). Стилевые правила
    приводят к тому, что линтер начинают глушить `noqa`, а вместе с вкусами глохнут и
    настоящие находки: страж, выключенный, чтобы не мешал, — не страж.
    (2) `target-version` равен САМОЙ СТАРОЙ заявленной версии Python. Проверяй линтер
    синтаксис версии разработчика — он пропустил бы код, который у пользователя не
    соберётся: та же дыра, что была у матрицы CI до 0.11.0.
    """
    pj = _read(ROOT / "pyproject.toml")
    m = re.search(r'requires-python = ">=(\d+)\.(\d+)"', pj)
    assert m, "requires-python не найден"
    expected = f'target-version = "py{m.group(1)}{m.group(2)}"'
    assert expected in pj, f"ruff должен целиться в самую старую заявленную версию: {expected}"

    rules = re.search(r'\[tool\.ruff\.lint\]\s*\nselect = \[([^\]]*)\]', pj)
    assert rules, "не найден select в [tool.ruff.lint]"
    selected = {r.strip().strip('"\'') for r in rules.group(1).split(",") if r.strip()}
    assert selected == {"E9", "F"}, (
        f"набор правил линтера разошёлся с задуманным (E9, F): {sorted(selected)}. "
        f"Расширять — только под конкретный воспроизведённый дефект, иначе линтер начнут глушить."
    )
